...
const startIdx = code.indexOf('export async function getFranchiseCollection');
  ...
  try {
     ...
     if (searchRes.ok) {
        ...
        if (data.results?.length > 1) {
           // Filter candidates: they must include the stem in their title
...
`;
  ...
  console.log("Rewrote getFranchiseCollection to NOT filter out empty paths in tmdbApi.ts");
} else {
